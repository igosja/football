...
function f_igosja_season_league_most_titled_record()
{
    global $igosja_season_id;

    $sql = "SELECT `leagueparticipant_tournament_id`
            FROM `leagueparticipant`
            WHERE `leagueparticipant_season_id`='$igosja_season_id'
            ORDER BY `leagueparticipant_tournament_id` ASC";
    ...
    for ($i=0; $i<$count_tournament; $i++)
    {
        ...
        $sql = "SELECT COUNT(`leagueparticipant_id`) AS `count`,
                ...
                AND `leagueparticipant_tournament_id`='$tournament_id'
                GROUP BY `leagueparticipant_team_id`
                ORDER BY `count` DESC
                LIMIT 1";
        ...
        $winner_id      = $winner_array[0]['leagueparticipant_team_id'];
