...
//Количество зрителей на трибунах
{
    ...
    $sql = "UPDATE `game`
            ...
            SET `game_visitor`=IF(ROUND((`home`.`team_visitor`+`guest`.`team_visitor`)*`tournament_visitor`)>`stadium_capacity`,`stadium_capacity`,ROUND(`home`.`team_visitor`+`guest`.`team_visitor`)*`tournament_visitor`),
    ...
    $sql = "UPDATE `game`
            ...
            LEFT JOIN `stadium`
            ON `stadium_id`=`game_stadium_id`
            SET `game_visitor`=IF(ROUND('50000'*`tournament_visitor`)>`stadium_capacity`, `stadium_capacity`, ROUND('50000')*`tournament_visitor`),
                `game_ticket_price`=IF(ROUND(`stadium_capacity`/'1000')>'10',ROUND(`stadium_capacity`/'1000'),'10')
            WHERE `shedule_date`=CURDATE()
            AND `game_played`='0'
            AND `game_home_team_id`='0'";
