...
$sql = "SELECT `broadcasting_minute`,
               `broadcasting_text`,
               `name_name`,
               `player_id`,
               `position_name`,
        ...
        FROM `broadcasting`
        LEFT JOIN `player`
        ON `broadcasting_player_id`=`player_id`
        LEFT JOIN `name`
        ...
        LEFT JOIN `team`
        ON `team_id`=`broadcasting_team_id`
        WHERE `broadcasting_game_id`='$get_num'
        AND `lineup_game_id`='$get_num'
        ORDER BY `broadcasting_id` DESC";
$broadcast_sql = $mysqli->query($sql);

$broadcast_array = $broadcast_sql->fetch_all(MYSQLI_ASSOC);
...
$smarty->assign('broadcast_array', $broadcast_array);
